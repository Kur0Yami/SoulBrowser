...
class DefaultCredentialProvider extends SystemEnvironmentProvider {
    ...
    public static class ComputeGoogleCredential extends GoogleCredential {
        ...
        public TokenResponse executeRefreshToken() {
            ...
            if (statusCode == 404) {
                throw new IOException(a.f(statusCode, "Error code ", " trying to get security access token from Compute Engine metadata for the default service account. This may be because the virtual machine instance does not have permission scopes specified."));
            }
    ...
    private final File getWellKnownCredentialsFile() {
        File file;
        if (getProperty("os.name", RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED).toLowerCase(Locale.US).indexOf("windows") >= 0) {
            file = new File(new File(getEnv("APPDATA")), CLOUDSDK_CONFIG_DIRECTORY);
        } else {
            file = new File(new File(getProperty("user.home", RequestConfiguration.MAX_AD_CONTENT_RATING_UNSPECIFIED), ".config"), CLOUDSDK_CONFIG_DIRECTORY);
        }
        return new File(file, WELL_KNOWN_CREDENTIALS_FILE);
    }

    private boolean runningOnCloudShell() {
        if (getEnv(CLOUD_SHELL_ENV_VAR) != null) {
            return true;
        }
        return false;
    }
    ...
    private boolean useGAEStandardAPI() {
        try {
            try {
                ...
                throw ((RuntimeException) OAuth2Utils.exceptionWithCause(new RuntimeException(a.C("Unexpcted error trying to determine if runnning on Google App Engine: ", e.getMessage())), e));
            }
    ...
    public final GoogleCredential getDefaultCredential(HttpTransport httpTransport, JsonFactory jsonFactory) {
        synchronized (this) {
            try {
